...
- **Wait-time happiness (§4)**: a `patienceThreshold()` (months) that's forgiving on a small network and tightens as it grows — `6 − 0.4·(stations−2)`, clamped to `[2, 6]` (6 at the 2-station start, 2 once all 12 are open). Each End Month tallies groups waiting past patience as **unhappy**; reputation loses `ceil(unhappy / 3)`. Groups that wait `ABANDON_GRACE` (3) months *beyond* patience give up and **leave the queue**. This **replaces** the old flat "−1 per overflowing queue" rep rule with a wait-driven one. Unhappy groups are flagged in both queue UIs (red border + "· unhappy"), and the monthly summary reports "Unhappy passengers: N (M left)".
- **Government subsidies (§7)**: serving an underserved district (Docks, Foundry, Newfield) pays a monthly, reputation-scaled bonus — `subsidyPerStation() = round(20 + rep·1.2)` per district actually served. "Served" = a group **boarded or alighted** there that month (a new `state.servedThisMonth` map set in `loadGroupsAtStation`/`unloadGroupsAtStation`, reset each End Month). `net = fares − costs + subsidy`; the summary shows a "Govt. subsidy (N districts)" line and each underserved station's detail advertises the ~$/mo it would earn. `state.servedThisMonth` is backfilled by `normalizeState` so old saves migrate.
- **Verified**: headless sim test (`financetest.js`, 24 checks — patience curve, unhappy/abandon tallies, rep math, subsidy gating + amount + net/cash, served-flag wiring through a real dispatch, and a clean-month baseline) plus a UI-path jsdom check (`uicheck.js`, 10 — desktop subsidy note, unhappy queue marker, and both new summary rows render with no errors; mobile helpers present). Season (20), desktop fleet (26), and mobile-load suites all still green. Harnesses in the session scratchpad, not committed.
